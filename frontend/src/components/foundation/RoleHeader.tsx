  ...
  return (
    <header className="role-header flex min-h-20 items-center justify-between gap-4 border-b border-white/7 px-4 py-4 sm:px-6 lg:px-10">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden text-right lg:block"><p className="text-xs font-medium text-slate-300">{user ? user.display_name : current.context}</p><p className="mt-1 text-[0.68rem] text-slate-500">{user ? `Signed in as ${user.role}` : 'No partner connection'}</p></div>
        ...
        <button type="button" onClick={signOut} className="rounded-lg border border-white/10 px-3 py-1.5 text-xs font-semibold text-slate-400 transition-colors hover:border-white/20 hover:text-slate-200">Sign out</button>
      </div>
    </header>
  )
}
